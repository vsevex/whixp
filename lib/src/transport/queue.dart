...
class _QueuedFuture<T> {
  final async.Completer completer;
  ...
  Future<void> execute() async {
    try {
      T result;
      async.Timer? timoutTimer;

      if (timeout != null) {
        timoutTimer = async.Timer(timeout!, () {
          _timedOut = true;
          if (onComplete != null) {
            onComplete?.call();
          }
...
class Queue {
  ...
  async.StreamController<int>? _remainingItemsController;
  ...
  Stream<int> get remainingItems {
    ...
    _remainingItemsController ??= async.StreamController<int>();
    return _remainingItemsController!.stream.asBroadcastStream();
  }

  final List<async.Completer<void>> _completeListeners = [];
