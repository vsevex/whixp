...
class Mishap extends Error {
  ...
  /// ```
  static Mishap? fromElement(xml.XmlElement element) {
    ...
    elements = element
        .findAllElements(Echotils.getNamespace('STREAM'), namespace: 'error')
        .toList();
    ...
    final error =
        Mishap(condition: condition, text: text, application: application);
  ...
  @override
  String toString() => '''$condition${text != null ? ' (text - $text)' : ''}''';
}
